...
class Scatterplot extends PureComponent {
    render() {
        ...
        return (
            <svg viewBox={`0 0 ${width} ${height}`} width={width} height={height} style={{ width: '100%', height: '100%' }}>
                <Layer data={points} xScale={xScale} yScale={yScale} minY={minY} maxY={maxY} maxX={maxX} />
            </svg>
        )
    }
}
...
class Layer extends Component {
    ...
    renderCircle = (key, data, style, dataIndex) => {
        return <circle key={key} r={style.r} cx={style.x} cy={style.y} fill={`hsl(${dataIndex % 360}, 100%, 71%)`} fillOpacity={style.opacity} />
    }
}
